...
export function ProjectHeaderButton({ project }: TProjectHeaderButtonProps) {
  return (
    <Tooltip label={project.name} layout="stacked" side="bottom">
      <div className="relative flex w-full max-w-48 items-center pe-1 text-start select-none">
        ...
        <div className="relative min-w-0 flex-1 hover:rounded">
          ...
          <div className="pointer-events-none absolute end-0 top-0 bottom-0 flex items-center justify-end opacity-0 transition-opacity duration-200 group-hover:opacity-100">
            <div className="relative flex h-full w-8 items-center justify-end">
              <div className="absolute inset-0 rounded-e bg-gradient-to-r from-transparent to-surface-2" />
