...
export function MaintenanceView({ onRetry }: { onRetry?: () => void }) {
  // hooks
        ...
        <div className="relative mt-4 flex w-full flex-col gap-4">
          ...
          {onRetry && (
            <button
              type="button"
              onClick={onRetry}
              className="self-start rounded-sm border border-subtle px-4 py-2 text-14 font-medium text-primary hover:bg-layer-1"
            >
              Retry connection
            </button>
          )}
